...
public interface IOsmosisRepository extends TokenPriceRepository<OsmosisTokenPrice, TokenPriceId> {
    ...
    @Query(value = "SELECT * FROM token_prices tp WHERE tp.base_denom = ?1 AND tp.zone = ?2 AND tp.osmosis_symbol_price_in_usd is null AND tp.datetime > (SELECT etp.datetime FROM token_prices etp WHERE etp.base_denom = ?1 AND tp.zone = ?2 AND etp.osmosis_symbol_price_in_usd is not null ORDER BY etp.datetime ASC LIMIT 1) ORDER BY tp.datetime ASC", nativeQuery = true)
